    ...
    while(menuLoop==true && casual == false )
    {
        while(SDL_PollEvent(&event))
        {
            ...
            case SDL_MOUSEMOTION:
                ...
                else if(mouseParRect(text.helpRect,mouseY) && indicator.selected != 2)
                {
                    sfx.playSelect();
                    indicator.setPosHelp();
                }
                else if(mouseParRect(text.quitRect,mouseY) && indicator.selected != 3)
                {
                    sfx.playSelect();
    ...
    while(gameReset==false)
    {
        while(SDL_PollEvent(&event))
        {
            ...
            case SDL_MOUSEBUTTONDOWN:
                ...
                {
                case SDL_BUTTON_LEFT:
                    if(mouseX>=590 && mouseX<=690 && mouseY>=445 && mouseY<=475)
                    {
                        gameReset = true;
                    }
                    break;
